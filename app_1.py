...
async def handle_incoming_call(request: Request):
    ...
    logger.info(f"Twilio connect stream URL: wss://{host}/media-stream")
...
async def handle_media_stream(websocket: WebSocket):
    ...
    async with websockets.connect(
        ...
        async def receive_from_twilio():
            nonlocal stream_sid
            try:
                async for message in websocket.iter_bytes():  # Change iter_text() to iter_bytes() for binary audio
                    data = json.loads(message.decode('utf-8'))
                    if data["event"] == "media":
                        logger.info("Received media from Twilio.")
                        audio_append = {
    ...
    session_update = {
        ...
        "session": {
            ...
            "instructions": (
                ...
                "Keep all your responses very concise and straight to the point and not more than 15 words."
